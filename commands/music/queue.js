    ...
    switch (subcommand) {
      case "list": {
        ...
        let queueList = `Currently playing: \n[${player.queue.current.title}](${player.queue.current.uri}) | ~${formatMS(
          ...
          true
        ).padStart(5, "00:")} left | <@${player.queue.current.requester.id}>\n\n`;
        for (var i = 0; i < player.queue.length; i++) {
          if (i === 0) {
            queueList += `**— Full list —**\n\n1. [${player.queue[i].title}](${player.queue[i].uri}) | ${formatMS(player.queue[i].duration, true).padStart(
              5,
              "00:"
            )} | <@${player.queue[i].requester.id}>\n`;
          } else {
            const nextItem = `${i + 1}. [${player.queue[i].title}](${player.queue[i].uri}) | ${formatMS(player.queue[i].duration, true).padStart(
              5,
              "00:"
            )} | <@${player.queue[i].requester.id}>\n`;
      ...
      case "remove": {
        ...
        if (removeIndex > player.queue.length)
          return embedReply(`Selected index ${removeIndex} out of range`, "Ensure the track index is within with queue list", "warn");

        try {
          const trackName = player.queue[removeIndex - 1].title;
          player.queue.remove(removeIndex - 1);
          embedReply("Track removed", trackName);
        } catch {
          embedReply("Failed to remove track", "Ensure the track index is correct with queue list", "warn");
        }
        break;
      }
      case "remove_many": {
        ...
        if (startRemoveIndex > player.queue.length || endRemoveIndex > player.queue.length)
          return embedReply(`Selected index ${startRemoveIndex}-${endRemoveIndex} out of range`, "Ensure the track index is within with queue list", "warn");

        try {
          player.queue.remove(startRemoveIndex - 1, endRemoveIndex);
          embedReply("Tracks removed");
        } catch {
          embedReply("Failed to remove tracks", "Ensure the tracks' index is correct with queue list", "warn");
        }
        break;
      }
    }
